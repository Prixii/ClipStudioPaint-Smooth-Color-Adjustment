...
void Texture::Resize(unsigned int newWidth, unsigned int newHeight, void* pixdata, unsigned int rowBytes)
{
	...
	else if (format == _format::float32)
		dataformat = DXGI_FORMAT::DXGI_FORMAT_R32_FLOAT;
...
void Texture::CopyToTexture(Texture& target)
{
	D3D11Graphic::GetContext()->CopyResource(target.pThis.Get(),pThis.Get());
...
void RenderTexture::Clear()
{
	if (pRt)
	{
		...
		D3D11Graphic::GetContext()->ClearRenderTargetView(pRt.Get(), clearColor);
	}
}
void RenderTexture::Resize(unsigned int newWidth, unsigned int newHeight)
{
	...
	textureDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	if (bRt)textureDesc.BindFlags |= D3D11_BIND_RENDER_TARGET;
	if (bUAV)textureDesc.BindFlags |= D3D11_BIND_UNORDERED_ACCESS;
	...
	if (bRt)
	{
		...
		D3D11Graphic::GetDevice()->CreateRenderTargetView(pThis.Get(), &rtDesc, &pRt);
	}
