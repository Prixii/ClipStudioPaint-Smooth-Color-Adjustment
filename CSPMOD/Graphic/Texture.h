...
{
public:
	RenderTexture(bool bRt = true, bool bUAV = false) :bRt(bRt), bUAV(bUAV) {};
	...
	void Bind();
	void UnBind();
	ID3D11RenderTargetView* GetRT() { return pRt.Get(); };
	ID3D11UnorderedAccessView* GetUAV() { return pUAV.Get(); };

	void Clear();
	//long_term:长期频繁拷贝，设置为true就不会清理频繁使用的资源。  只拷贝一次的设置为false
...
protected:
	...
	Microsoft::WRL::ComPtr<ID3D11RenderTargetView> pRt;
